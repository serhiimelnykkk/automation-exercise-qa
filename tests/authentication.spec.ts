...
test.describe("Authentication UI Tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.route(
      /(google-analytics|doubleclick|googlesyndication|adnxs)/,
      (route) => route.abort(),
    );
  });

  test("TC-AUTH-1: User signs up successfully with all fields", async ({
    loginPage,
    signupPage,
    accountCreatedPage,
    page,
    ...
    await expect(loginPage.loggedInAsText).toContainText(
      `Logged in as ${testData.validFullUser.name}`,
    );
  });

  test("TC-AUTH-2: User signs up successfully with only required fields", async ({
    loginPage,
    signupPage,
    accountCreatedPage,
    page,
    ...
    await loginPage.fillSignupForm(
      testData.validRequiredUser.name,
      uniqueEmail,
    );
    await loginPage.clickSignup();
    ...
    await expect(loginPage.loggedInAsText).toContainText(
      `Logged in as ${testData.validRequiredUser.name}`,
    );
  });

  test("TC-AUTH-3: Leaving all fields empty on step 1 doesn’t sign up a user", async ({
    loginPage,
    page,
  }) => {
    await loginPage.navigateToLogin();
    ...
    await expect(loginPage.signupNameInput).toHaveJSProperty(
      "validity.valid",
      false,
    );
    await expect(loginPage.signupEmailInput).toHaveJSProperty(
      "validity.valid",
      false,
    );
  ...
  test("TC-AUTH-4: Leaving all fields empty on step 2 doesn’t sign up a user", async ({
    loginPage,
    signupPage,
    page,
  }) => {
    ...
    await expect(signupPage.passwordInput).toHaveJSProperty(
      "validity.valid",
      false,
    );
    await expect(page).toHaveURL(/\/signup$/);
  ...
  test("TC-AUTH-5: Leaving email field empty on step 1 doesn’t signup a user", async ({
    loginPage,
    page,
  }) => {
    await loginPage.navigateToLogin();
  ...
  test("TC-AUTH-6: Leaving name field empty on step 1 doesn’t signup a user", async ({
    loginPage,
    page,
  }) => {
    await loginPage.navigateToLogin();
  ...
  for (const { id, field, desc } of step2RequiredFields) {
    test(`${id}: Leaving ${desc} field empty on step 2 doesn’t sign up a user`, async ({
      loginPage,
      signupPage,
      page,
    }) => {
      ...
      const locator = signupPage[
        `${field}Input` as keyof typeof signupPage
      ] as Locator;
  ...
  test("TC-AUTH-13: Leaving Country field empty on step 2 doesn’t sign up a user", async ({
    loginPage,
    signupPage,
    page,
  }) => {
    ...
    await expect(signupPage.countrySelect).toHaveJSProperty(
      "validity.valid",
      true,
    );
  });

  test("TC-AUTH-16: Email validates format on signup form", async ({
    loginPage,
    page,
  }) => {
    await loginPage.navigateToLogin();
  ...
  test.describe("Login & Login Validations", () => {
    ...
    test("TC-AUTH-17: User logs in with correct credentials", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
      ...
      await expect(page).toHaveURL(/\/$/);
      await expect(loginPage.loggedInAsText).toContainText(
        `Logged in as ${testData.loginCredentials.name}`,
      );
    });

    test("TC-AUTH-18: Leaving all fields empty on login form doesn’t log in a user", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
      ...
      await expect(loginPage.loginEmailInput).toHaveJSProperty(
        "validity.valid",
        false,
      );
      await expect(loginPage.loginPasswordInput).toHaveJSProperty(
        "validity.valid",
        false,
      );
      await expect(page).toHaveURL(/\/login$/);
    ...
    test("TC-AUTH-19: Leaving password field empty on login form doesn’t log in a user", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
    ...
    test("TC-AUTH-20: Leaving email field empty on login form doesn’t log in a user", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
    ...
    test("TC-AUTH-21: Email validates format on login form", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
    ...
    test("TC-AUTH-22: User login fails with incorrect password", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
    ...
    test("TC-AUTH-23: User login fails with unregistered email", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
      ...
      await loginPage.fillLoginForm(
        "unregistered_user_12345@example.com",
        "SomePassword123",
      );
      await loginPage.clickLogin();
    ...
    test("TC-AUTH-24: User signup fails with an already registered email address", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
      ...
      await loginPage.fillSignupForm(
        testData.loginCredentials.name,
        testData.loginCredentials.email,
      );
      await loginPage.clickSignup();

      await expect(loginPage.signupErrorText).toHaveText(
        "Email Address already exist!",
      );
      await expect(page).toHaveURL(/\/signup$/);
    });

    test("TC-AUTH-25: User logs out successfully", async ({
      loginPage,
      page,
    }) => {
      await loginPage.navigateToLogin();
    ...
    test("TC-AUTH-26: User deletes account via UI", async ({
      loginPage,
      accountDeletedPage,
      page,
      userApiClient,
